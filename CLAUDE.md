...
Three related features, all hidden per-location until an admin turns them on in **Admin → Locations** (`show_budget_tracking`, `show_ownership_tools`). Data entry and reporting are split across three pages:

- **Site Entry (`Dashboard.jsx`)** — daily numbers, entered by the site itself. When a site has the toggle on, "Budget Tracking" and/or "Ownership Log" appear as extra tabs next to Daily Log / Snapshot / Rollup, using `BudgetTrackingSection.jsx` (Daily Entry) and `OwnershipLogSection.jsx` (today's post, editable for up to 3 days after submitting). Each tab shows a small orange/red dot (`morningStatus.js`) when that day's entry is still outstanding.
- **AM Entry (`AmEntry.jsx`)** — admin/area-manager only (NavBar button next to Site Entry). This is where monthly, cross-site management data lives: the **Ownership Scorecard** (`OwnershipScorecardSection.jsx`, `canManage` always on here) and **Budget Targets** for every site in one table (`BudgetTargetsTable.jsx`), both edited via modal. An area manager's sites populate automatically (their `useAuth().locations`, already scoped via `manager_locations`); an admin sees every site with the feature on, assigned or not. A "Recent edits" panel (`EditAuditLog.jsx`) at the bottom shows the audit trail (see below).
- **Reports (`Reports.jsx`)** — read-only, admin/area-manager only, no data entry anywhere on this page (including Scorecard and Targets, which used to be editable here — that moved to AM Entry). `BudgetTrackingReport.jsx` shows the cross-site leaderboard with a Daily-Entry-only edit modal per site; `OwnershipLogReport.jsx` shows each site's latest post and status with its own admin-override edit modal (no 3-day limit).

Shared pieces:
- `budgetMath.js`: Yesterday/MTD derived metrics, revenue pace, membership pace/bonus tiers, and the Leaderboard score (Yesterday Conv 25%, MTD Conv 25%, MTD P-Mix 15%, membership progress 25%, rating 10% — the documented workbook weights, not the source sheet's buggy live formula). MTD Membership Actual is typed in directly (`budget_daily_entries.mtd_membership_actual`), same as the rest of the daily figures. `monthProgress()`/`daysElapsed()` measure through **yesterday**, not today — Yesterday/MTD figures are entered each morning for the prior day, so on the 15th of a 30-day month progress is 14/30 (46.7%), not 15/30. Membership Goal is derived, not typed: a site's `starting_members` + `membership_daily_growth` (`budget_targets`, default 2/day, edited on AM Entry) compute both the end-of-month goal (bonus tiers) and a day-by-day Membership Pace badge, mirroring Revenue Pace.
- `morningStatus.js`: the shared "is today's entry done yet" status used by Budget Tracking and Ownership Log alike — `none` before 6am local, `pending` (orange) 6–10am, `overdue` (red) after 10am, `complete` once submitted. Fixed schedule, not per-site configurable (`ownership_log_cutoff_time` on `locations` is unused now).
- **Ownership Scorecard**: monthly 10-category manager rating (each 0–10, shown as a single overall percentage out of 100%). Column headers are pre-wrapped to exactly 2 lines each (`lines: [...]` per category in `OwnershipScorecardSection.jsx`, not CSS auto-wrap) at a fixed width sized for "Dependability", the longest single word. A Top 5 / Bottom 5 performers summary sits above the table — click a listed site to reveal its strongest (top list) or weakest (bottom list) category.
- **Audit log** (`edit_audit_log` table, `auditLog.js`, `EditAuditLog.jsx`): every Budget Target and Ownership Scorecard save logs a diff — who changed which fields, old → new, and when — visible on AM Entry's "Recent edits" panel. An edit made after the 5th of the month is flagged as a mid-month edit. Append-only by RLS policy (no update/delete).
